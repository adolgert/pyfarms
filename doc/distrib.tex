...
Where theoretical bounds are known, they will be used to verify
...
In the absence of these measures, for some distributions,
the Kolmogorov-Smirnov test will compare samples with those
from a known implementation. Each distribution has to be tested
for sampling two different ways, once with simple sampling,
and once with the kind of sampling used for Anderson's method,
which relies on hazard integrals and inverse hazard integrals.
